...
#ifndef NIRT_I_EVENT_RECEIVER_H_INCLUDED
#define NIRT_I_EVENT_RECEIVER_H_INCLUDED
...
namespace nirt
{
	...
	enum EEVENT_TYPE
	{
		...
		//! A mouse input event.
		/** Mouse events are created by the device and passed to NirtcppDevice::postEventFromUser
		...
		*/
		...
		NirtcppDevice::postEventFromUser. They take the same path as mouse events. */
		EET_KEY_INPUT_EVENT,
		...
		device run() and then passing the events to NirtcppDevice::postEventFromUser.
		...
		*/
		...
		/** This is not used by Nirtcpp and can be used to send user
		specific data though the system. The Nirtcpp 'window handle'
		can be obtained from NirtcppDevice::getExposedVideoData()
		The usage and behavior depends on the operating system:
		Windows: send a WM_USER message to the Nirtcpp Window; the
			wParam and lParam will be used to populate the
			UserData1 and UserData2 members of the SUserEvent.
		Linux: send a ClientMessage via XSendEvent to the Nirtcpp
...
struct SEvent
{
	 ...
	 * active device, _NIRT_COMPILE_WITH_JOYSTICK_EVENTS_ is defined, and
	 * @ref nirt::NirtcppDevice::activateJoysticks() has been called, an event of
	 * this type will be generated once per joystick per @ref NirtcppDevice::run()
	...
	struct SJoystickEvent
	{
		enum
		{
			...
			AXIS_U,
			AXIS_V,
			NUMBER_OF_AXES=18	// (please tell Nirtcpp maintainers if you absolutely need more axes)
		};
		...
		/** This is an internal Nirtcpp index; it does not map directly
...
nirt::EEVENT_TYPE for a description of where each type of event starts, and the
...
public:
	...
	/** Please take care that you should only return 'true' when you want to _prevent_ Nirtcpp
...
//! Information on a joystick, returned from @ref nirt::NirtcppDevice::activateJoysticks()
struct SJoystickInfo
{
	//! The ID of the joystick
	/** This is an internal Nirtcpp index; it does not map directly
	 * to any particular hardware joystick. It corresponds to the
	 * nirt::SJoystickEvent Joystick ID. */
	u8 Joystick;
