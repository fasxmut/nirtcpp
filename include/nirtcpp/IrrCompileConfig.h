...
//! _NIRT_WINDOWS_ for all nirtcpp supported Windows versions
//! _NIRT_WINDOWS_API_ for Windows or XBox
//! _NIRT_LINUX_PLATFORM_ for Linux (it is defined here if no other os is defined)
//! _NIRT_SOLARIS_PLATFORM_ for Solaris
//! _NIRT_OSX_PLATFORM_ for Apple systems running OSX
//! _NIRT_POSIX_API_ for Posix compatible systems
...
//! Nirtcpp can be compiled with more than one device
//! _NIRT_COMPILE_WITH_WINDOWS_DEVICE_ for Windows API based device
//! _NIRT_COMPILE_WITH_OSX_DEVICE_ for Cocoa native windowing on OSX
//! _NIRT_COMPILE_WITH_X11_DEVICE_ for Linux X11 based device
//! _NIRT_COMPILE_WITH_SDL_DEVICE_ for platform independent SDL framework
//! _NIRT_COMPILE_WITH_CONSOLE_DEVICE_ for no windowing system, used as a fallback
//! _NIRT_COMPILE_WITH_FB_DEVICE_ for framebuffer systems

//! Passing defines to the compiler which have NO in front of the _NIRT definename is an alternative
...
//! Example: NO_NIRT_COMPILE_WITH_X11_ would disable X11


//! Uncomment this line to compile with the SDL device
//#define _NIRT_COMPILE_WITH_SDL_DEVICE_
#ifdef NO_NIRT_COMPILE_WITH_SDL_DEVICE_
#undef _NIRT_COMPILE_WITH_SDL_DEVICE_
#endif
...
// XBox is deprecated (as DX8 is removed). Use Nirtcpp 1.8 if you still want to work on this.
	...
	#define _NIRT_XBOX_PLATFORM_	// deprecated
	#define _NIRT_WINDOWS_API_
	//#define _NIRT_COMPILE_WITH_WINDOWS_DEVICE_
...
//! Define _NIRT_COMPILE_WITH_JOYSTICK_SUPPORT_ if you want joystick events.
...
//! Maximum number of texture an SMaterial can have, up to 8 are supported by Nirtcpp.
...
//! Add a leak-hunter to Nirtcpp which helps finding unreleased reference counted objects.
...
//! Define _NIRT_COMPILE_WITH_DIRECT3D_9_ to compile the Nirtcpp engine with DIRECT3D9.
...
_NIRT_COMPILE_WITH_DX9_DEV_PACK_. So you simply need to add something like this
to the compiler settings: -DNIRT_COMPILE_WITH_DX9_DEV_PACK
...
//! Define _NIRT_COMPILE_WITH_DIRECTINPUT_JOYSTICK_ if you want to use DirectInput for joystick handling.
...
//! Define _NIRT_COMPILE_WITH_OPENGL_ to compile the Nirtcpp engine with OpenGL.
...
//! Define required options for OpenGL drivers.
		...
		#define _NIRT_COMPILE_WITH_GLX_MANAGER_
	#elif defined(_NIRT_COMPILE_WITH_SDL_DEVICE_)
		#define _NIRT_OPENGL_USE_EXTPOINTER_
	#endif
#endif
...
//! Define _NIRT_COMPILE_WITH_SOFTWARE_ to compile the Nirtcpp engine with software driver
...
//! Define _NIRT_COMPILE_WITH_BURNINGSVIDEO_ to compile the Nirtcpp engine with Burning's video driver
...
//! Define _NIRT_COMPILE_WITH_X11_ to compile the Nirtcpp engine with X11 support.
...
//! Define _NIRT_COMPILE_WITH_GUI_ to compile the engine with the built-in GUI
...
//! Define _NIRT_COMPILE_WITH_PARTICLES to compile the engine the withe build-in particle system
...
//! Define _NIRT_WCHAR_FILESYSTEM to enable unicode filesystem support for the engine.
...
//! Define _NIRT_COMPILE_WITH_LIBJPEG_ to enable compiling the engine using libjpeg.
...
//! Define _NIRT_USE_NON_SYSTEM_JPEG_LIB_ to let nirtcpp use the jpeglib which comes with nirtcpp.
/** If this is commented out, Nirtcpp will try to compile using the jpeg lib installed in the system.
	This is only used when _NIRT_COMPILE_WITH_LIBJPEG_ is defined.
...
//! Define _NIRT_COMPILE_WITH_LIBPNG_ to enable compiling the engine using libpng.
...
//! Define _NIRT_USE_NON_SYSTEM_LIBPNG_ to let nirtcpp use the libpng which comes with nirtcpp.
/** If this is commented out, Nirtcpp will try to compile using the libpng installed in the system.
	This is only used when _NIRT_COMPILE_WITH_LIBPNG_ is defined.
	NOTE: You will also have to modify the Makefile or project files when changing this default.
	*/
...
//! Define _NIRT_D3D_NO_SHADER_DEBUGGING to disable shader debugging in D3D9
/** If _NIRT_D3D_NO_SHADER_DEBUGGING is undefined in IrrCompileConfig.h,
...
//! Define _NIRT_D3D_USE_LEGACY_HLSL_COMPILER to enable the old HLSL compiler in recent DX SDKs
...
//! Define _NIRT_USE_NVIDIA_PERFHUD_ to opt-in to using the nVidia PerHUD tool
...
/** So if we were marketing guys we could say Nirtcpp has 4 Software-Rasterizers.
	In a Nutshell:
...
//! Define _NIRT_COMPILE_WITH_SHADOW_VOLUME_SCENENODE_ to support ShadowVolumes
...
//! Define _NIRT_COMPILE_WITH_OCTREE_SCENENODE_ to support OctreeSceneNodes
...
//! Define _NIRT_COMPILE_WITH_TERRAIN_SCENENODE_ to support TerrainSceneNodes
...
//! Define _NIRT_COMPILE_WITH_BILLBOARD_SCENENODE_ to support BillboardSceneNodes
...
//! Define _NIRT_COMPILE_WITH_WATER_SURFACE_SCENENODE_ to support WaterSurfaceSceneNodes
...
//! Define _NIRT_COMPILE_WITH_SKYDOME_SCENENODE_ to support SkydomeSceneNodes
...
//! Define _NIRT_COMPILE_WITH_CUBE_SCENENODE_ to support CubeSceneNodes
...
//! Define _NIRT_COMPILE_WITH_SPHERE_SCENENODE_ to support CubeSceneNodes
...
//! Define _NIRT_COMPILE_WITH_NIRT_SCENE_LOADER_ if you want to be able to load
...
//! Define _NIRT_COMPILE_WITH_SKINNED_MESH_SUPPORT_ if you want to use bone based
...
#endif
//! Define _NIRT_COMPILE_WITH_HALFLIFE_LOADER_ if you want to load Halflife animated files
...
#endif
//! Define _NIRT_COMPILE_WITH_DDS_DECODER_LOADER_ if you want to load .dds files
...
//! Define _NIRT_COMPILE_WITH_ZLIB_ to enable compiling the engine using zlib.
...
#endif
//! Define _NIRT_USE_NON_SYSTEM_ZLIB_ to let nirtcpp use the zlib which comes with nirtcpp.
/** If this is commented out, Nirtcpp will try to compile using the zlib
	installed on the system. This is only used when _NIRT_COMPILE_WITH_ZLIB_ is
	defined.
	NOTE: You will also have to modify the Makefile or project files when changing this default.
 */
...
#endif
//! Define _NIRT_COMPILE_WITH_ZIP_ENCRYPTION_ if you want to read AES-encrypted ZIP archives
...
#endif
//! Define _NIRT_COMPILE_WITH_BZIP2_ if you want to support bzip2 compressed zip archives
...
#endif
//! Define _NIRT_USE_NON_SYSTEM_BZLIB_ to let nirtcpp use the bzlib which comes with nirtcpp.
/** If this is commented out, Nirtcpp will try to compile using the bzlib
installed on the system. This is only used when _NIRT_COMPILE_WITH_BZLIB_ is
defined.
NOTE: You will also have to modify the Makefile or project files when changing this default.
*/
...
#endif
//! Define _NIRT_COMPILE_WITH_LZMA_ if you want to use LZMA compressed zip files.
/** LZMA is a very efficient compression code, known from 7zip. Nirtcpp
...
//! Define __NIRT_COMPILE_WITH_MOUNT_ARCHIVE_LOADER_ if you want to mount folders as archives
...
#endif
//! Define __NIRT_COMPILE_WITH_PAK_ARCHIVE_LOADER_ if you want to open ID software PAK archives
...
#endif
//! Define __NIRT_COMPILE_WITH_NPK_ARCHIVE_LOADER_ if you want to open Nebula Device NPK archives
...
#endif
//! Define __NIRT_COMPILE_WITH_TAR_ARCHIVE_LOADER_ if you want to open TAR archives
...
#endif
//! Define __NIRT_COMPILE_WITH_WAD_ARCHIVE_LOADER_ if you want to open WAD archives
...
//! Set FPU settings
/** Nirtcpp should use approximate float and integer fpu techniques
...
#ifdef _DEBUG
	//! A few attributes are written in CSceneManager when _NIRT_SCENEMANAGER_DEBUG is enabled
...
#endif // NIRT_COMPILE_CONFIG_H_INCLUDED
