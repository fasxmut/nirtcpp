...
try {
	...
	nirt::scene::IAnimatedMesh * ninja_mesh = nullptr; // Mesh model on disk.
	if (ninja_mesh = smgr->getMesh("../../../media/ninja.b3d"); ninja_mesh == nullptr)
	if (ninja_mesh = smgr->getMesh("../../media/ninja.b3d"); ninja_mesh == nullptr)
	if (ninja_mesh = smgr->getMesh("../media/ninja.b3d"); ninja_mesh == nullptr)
	if (ninja_mesh = smgr->getMesh("./media/ninja.b3d"); ninja_mesh == nullptr)
		ninja_mesh = smgr->getMesh("./ninja.b3d");
	...
	if (!ninja_node)
		std::cerr << "Can not load ninja mesh!\n";
