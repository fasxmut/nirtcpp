...
IImage* CImageLoaderPPM::loadImage(io::IReadFile* file) const
{
	...
	const u32 width = core::strtoul10(token.data());

	getNextToken(file, token);
	const u32 height = core::strtoul10(token.data());
	...
	{
		getNextToken(file, token);
		const u32 maxDepth = core::strtoul10(token.data());
		...
		if (format==2 || format==5)
		{
			...
			else
			{
				...
				if (image)
				{
					...
					{
						getNextToken(file, token);
						const u8 num = (u8)core::strtoul10(token.data());
						*ptr++ = num;
						*ptr++ = num;
						*ptr++ = num;
						*ptr++ = 255;
		...
		else
		{
			...
			else
			{
				...
				if (image)
				{
					...
					{
						getNextToken(file, token);
						*ptr++ = (u8)core::strtoul10(token.data());
						getNextToken(file, token);
						*ptr++ = (u8)core::strtoul10(token.data());
						getNextToken(file, token);
						*ptr++ = (u8)core::strtoul10(token.data());
