...
BOOL WINAPI ConsoleHandler(DWORD CEvent)
{
    ...
    {
    case CTRL_C_EVENT:
		nirt::os::Printer::log("Closing console device", "CTRL+C");
		break;
	case CTRL_BREAK_EVENT:
		nirt::os::Printer::log("Closing console device", "CTRL+Break");
		break;
    case CTRL_CLOSE_EVENT:
		nirt::os::Printer::log("Closing console device", "User closed console");
		break;
    case CTRL_LOGOFF_EVENT:
		nirt::os::Printer::log("Closing console device", "User is logging off");
		break;
    case CTRL_SHUTDOWN_EVENT:
		nirt::os::Printer::log("Closing console device", "Computer shutting down");
		break;
    }
	DeviceToClose->closeDevice();
    return TRUE;
}
#elif defined(_NIRT_POSIX_API_)
// sigterm handler
#include <signal.h>

void sighandler(int sig)
{
	nirt::core::stringc code = "Signal ";
	code += sig;
	code += " received";
	nirt::os::Printer::log("Closing console device", code.data());
...
//! constructor
CIrrDeviceConsole::CIrrDeviceConsole(const SNirtcppCreationParameters& params)
...
#elif defined(_NIRT_VT100_CONSOLE_)
	// send escape code
