...
#ifndef NIRT_C_NIRT_C_TIMER_H_INCLUDED
#define NIRT_C_NIRT_C_TIMER_H_INCLUDED
...
namespace nirt
{
	...
	public:
		...
		virtual u32 getRealTime() const override
		{
			return os::Timer::getRealTime();
		}

		//! Get current time and date in calendar form
		virtual RealTimeDate getRealTimeAndDate() const override
		{
		...
		virtual u32 getTime() const override
		{
			return os::Timer::getTime();
		}

		//! sets current virtual time
		virtual void setTime(u32 time) override
		{
		...
		real system time. */
		virtual void setSpeed(f32 speed = 1.0f) override
		{
		...
		/** The speed is the factor with which the time is running faster or slower then the
		real system time. */
		virtual f32 getSpeed() const override
		{
		...
		called automatically when calling NirtcppDevice::run(), but you can call it manually
