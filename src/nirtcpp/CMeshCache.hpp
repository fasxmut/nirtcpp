...
#ifndef NIRT_C_MESH_CACHE_H_INCLUDED
#define NIRT_C_MESH_CACHE_H_INCLUDED
...
namespace scene
{
	...
	public:
		...
		virtual void addMesh(const io::path& filename, IAnimatedMesh* mesh) override;

		//! Removes a mesh from the cache.
		/** After loading a mesh with getMesh(), the mesh can be removed from the cache
		using this method, freeing a lot of memory. */
		virtual void removeMesh(const IMesh* const mesh) override;
		...
		virtual u32 getMeshCount() const override;

		//! Returns current index number of the mesh, and -1 if it is not in the cache.
		virtual s32 getMeshIndex(const IMesh* const mesh) const override;
		...
		virtual IAnimatedMesh* getMeshByIndex(u32 index) override;

		//! Returns a mesh based on its name.
		/** \param name Name of the mesh. Usually a filename.
		\return Pointer to the mesh or 0 if there is none with this number. */
		virtual IAnimatedMesh* getMeshByName(const io::path& name) override;

		//! Get the name of a loaded mesh, based on its index.
		/** \param index: Index of the mesh, number between 0 and getMeshCount()-1.
		\return The name if mesh was found and has a name, else	the path is empty. */
		virtual const io::SNamedPath& getMeshName(u32 index) const override;

		//! Get the name of a loaded mesh, if there is any.
		/** \param mesh Pointer to mesh to query.
		\return The name if mesh was found and has a name, else	the path is empty. */
		virtual const io::SNamedPath& getMeshName(const IMesh* const mesh) const override;
		...
		virtual bool renameMesh(u32 index, const io::path& name) override;
		...
		\param name New name for the mesh.
		\return True if mesh was renamed. */
		virtual bool renameMesh(const IMesh* const mesh, const io::path& name) override;

		//! returns if a mesh already was loaded
		virtual bool isMeshLoaded(const io::path& name) override;

		//! Clears the whole mesh cache, removing all meshes.
		virtual void clear() override;

		//! Clears all meshes that are held in the mesh cache but not used anywhere else.
		virtual void clearUnusedMeshes() override;
