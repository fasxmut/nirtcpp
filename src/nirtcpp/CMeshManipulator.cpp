...
void recalculateNormalsT(IMeshBuffer* buffer, bool smooth, bool angleWeighted)
{
	...
	else
	{
		...
		for ( i=0; i<idxcnt; i+=3)
		{
			...
			if (angleWeighted)
				weight = nirt::scene::getAngleWeight(v1,v2,v3); // writing nirt::scene:: necessary for borland
...
void recalculateTangentsT(IMeshBuffer* buffer, bool recalculateNormals, bool smooth, bool angleWeighted)
{
	...
	if (smooth)
	{
		...
		for ( i=0; i<idxCnt; i+=3)
		{
			...
			core::vector3df weight(1.f,1.f,1.f);
			if (angleWeighted)
				weight = nirt::scene::getAngleWeight(v[i+0].Pos,v[i+1].Pos,v[i+2].Pos);	// writing nirt::scene:: necessary for borland
