...
#ifndef NIRT_C_DEFAULT_GUI_ELEMENT_FACTORY_H_INCLUDED
#define NIRT_C_DEFAULT_GUI_ELEMENT_FACTORY_H_INCLUDED
...
namespace gui
{
	...
	public:
		...
		virtual IGUIElement* addGUIElement(EGUI_ELEMENT_TYPE type, IGUIElement* parent=0) override;

		//! Adds a GUI element to the GUI Environment based on its type name.
		/** \param typeName: Type name of the element to add. Taken from the GUIElementTypeNames c8* array.
		\param parent: Parent scene node of the new element. A value of 0 adds it to the root.
		\return Returns pointer to the new element or 0 if unsuccessful. */
		virtual IGUIElement* addGUIElement(const c8* typeName, IGUIElement* parent=0) override;

		//! Returns the amount of GUI element types this factory is able to create.
		virtual s32 getCreatableGUIElementTypeCount() const override;
		...
		virtual EGUI_ELEMENT_TYPE getCreateableGUIElementType(s32 idx) const override;

		//! Returns the type name of a createable GUI element type based on the index.
		/** \param idx: Index of the element type in this factory. The value must be equal or greater than 0
		and lower than getCreatableGUIElementTypeCount(). */
		virtual const c8* getCreateableGUIElementTypeName(s32 idx) const override;
		...
		virtual const c8* getCreateableGUIElementTypeName(EGUI_ELEMENT_TYPE type) const override;
...
#endif // NIRT_C_DEFAULT_GUI_ELEMENT_FACTORY_H_INCLUDED
